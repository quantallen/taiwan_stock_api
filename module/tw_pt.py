...
class TF_pair_trading:
    ...
    Ref_SeqNum = 0
    Target_SeqNum = 0
    def init(self, api, order_api, config):
    ...
    async def check_trading(self,task_queue):
        self.spread_prices = self.predictor.get_target_spread_price(
                                ...
                                orderbook_5min= self.orderbook_1min,
    ...
    async def Update_orderbook(self,api,symbol):
        api.quote.subscribe(
        api.Contracts.Stocks[symbol],
        quote_type = sj.constant.QuoteType.BidAsk, # or 'bidask'
        version = sj.constant.QuoteVersion.v1 # or 'v1'
    )
        api.quote.set_on_bidask_fop_v1_callback(self.quote_callback)
